...
parser.add_argument('--gamma', type=float, default=0.5, help='Lb constant regularization loss weight [default: 0.5]')
parser.add_argument('--delta', type=float, default=1/30, help='Lb linear regularization loss weight [default: 1/30]')
...
parser.add_argument('--log_dir', default='log/SampleNetProgressive', help='Log dir [default: log/SampleNetProgressive]')
...
os.system("cp train_samplenet_progressive.py %s" % (LOG_DIR))  # bkp of train procedure
    ...
    with tf.Graph().as_default():
        with tf.device("/gpu:" + str(GPU_INDEX)):
            ...
            with tf.variable_scope("classifier"):
                ...
                projected_points_slice = tf.slice(
                    projected_points, [0, 0, 0], [BATCH_SIZE, pc_size, 3]
                )
                pred, end_points = CLASSIFIER_MODEL.get_model(
                    projected_points_slice, is_training_classifier_pl, bn_decay=bn_decay
                )
                loss_classifier = CLASSIFIER_MODEL.get_loss(pred, labels_pl, end_points)

                simplified_points_slice = tf.slice(
                    simplified_points, [0, 0, 0], [BATCH_SIZE, pc_size, 3]
                )
                loss_simplification = SAMPLER_MODEL.get_simplification_loss(
                    pointclouds_pl, simplified_points_slice, pc_size, GAMMA, DELTA
                )
            ...
            for pc_size in pc_sizes:
                ...
                with tf.variable_scope("classifier" + str(pc_size)):
                    pred, end_points = CLASSIFIER_MODEL.get_model(
                        projected_points_slice,
                        is_training_classifier_pl,
                        bn_decay=bn_decay,
                    )
                    loss_curr_classifier = CLASSIFIER_MODEL.get_loss(
                        pred, labels_pl, end_points
                    )
                    loss_classifier = tf.add(loss_classifier, loss_curr_classifier)

                    loss_simplification_curr = SAMPLER_MODEL.get_simplification_loss(
                        pointclouds_pl, simplified_points_slice, pc_size, GAMMA, DELTA
                    )

                    loss_simplification = tf.add(
                        loss_simplification, loss_simplification_curr
                    )
        ...
        no_scope_param.append(
            tf.get_default_graph().get_tensor_by_name("beta1_power:0")
        )
        no_scope_param.append(
            tf.get_default_graph().get_tensor_by_name("beta2_power:0")
        )
