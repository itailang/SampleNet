...
class PointNetAE(AutoEncoder):
    ...
    def __init__(self, name, configuration, graph=None):
        ...
        with tf.variable_scope(name):
            if c.exists_and_is_not_none("use_fps") and c.use_fps:
                self.idx = farthest_point_sample(
                    c.n_sample_points, self.x
                )  # (batch_size, n_sample_points)
                self.s = gather_point(
                    self.x, self.idx
                )  # (batch_size, n_sample_points, 3)
            else:
                self.s = self.x[
                    :, : c.n_sample_points, :
                ]  # (batch_size, n_sample_points, 3)
            ...
            idx_fps = farthest_point_sample(
                n_output_points, self.x_reconstr
            )  # (batch_size, n_output_points)
            self.x_reconstr_sorted = gather_point(
                self.x_reconstr, idx_fps
            )  # (batch_size, n_output_points, 3)
    ...
    def gradient_of_input_wrt_loss(self, in_points, gt_points=None):
        ...
        return self.sess.run(
            tf.gradients(self.loss, self.x),
            feed_dict={self.x: in_points, self.gt: gt_points},
        )
