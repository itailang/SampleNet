...
from reconstruction.src.samplenet_pointnet_ae import SampleNetPointNetAE
...
parser.add_argument('--train_folder', type=str, default='log/SampleNet64', help='Folder for loading data form the training [default: log/SampleNet64]')
...
reset_tf_graph()
ae = SampleNetPointNetAE(conf.experiment_name, conf)
...
)  # SampleNet sampled points

# save sampled point cloud and sample index
file_name = (
    "_".join(["sampled_pc", "test_set", flags.object_class, "%04d" % n_sample_points])
    + ".npy"
)
...
file_name = (
    "_".join(["sample_idx", "test_set", flags.object_class, "%04d" % n_sample_points])
    + ".npy"
)
file_path = osp.join(eval_dir, file_name)
...
if flags.visualize_results:
    ...
    plot_3d_point_cloud(
        sampled_pc[i], in_u_sphere=True, title="SampleNet sampled points"
    )
    plot_3d_point_cloud(
        reconstructions[i],
        in_u_sphere=True,
        title="Reconstruction from SampleNet points",
    )
